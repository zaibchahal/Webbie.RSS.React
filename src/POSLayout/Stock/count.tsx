...
import { AppConst, AppEnums, BASE_URL, STOCKCOUNT_URLS } from '../../common/RssData/constants';
...
import { creatUpdate, get, getList as getListStocks, IStockCountDetailProps, IStockCountFilterProps, IStockCountProps } from '../../services/POS/stokCountService';
...
const StockCount = () => {
    ...
    const [StockCounts, setStockCounts] = useState<IStockCountProps[]>([]);
    ...
    const handleAddNewRow = async (e: any, formik: FormikProps<IStockCountProps>) => {
        if (e.key === 'Enter') {
            ...
            currentDetails.push({
                ...
                productName: p.name,
                counted: 0,
                expected: p.balance || 0
            } as IStockCountDetailProps);
            formik.setValues({
                ...formik.values,
    ...
    const handleRemoveRow = (i: number, formik: FormikProps<IStockCountProps>) => {
        const updatedRows = formik.values.detail.filter((_, index) => index !== i);
        formik.setValues({ ...formik.values, detail: updatedRows });
    };

    const handleImageChange = async (e: any, formik: FormikProps<IStockCountProps>) => {
        setIsLoading(true);
        const file = e.target.files[0];
        const pic = await uploadFile({ file: file, prevFile: formik.values.file, subFolder: AppConst.SubFolders.StockCount } as IFileProp);
        formik.setFieldValue("file", pic);
        setIsLoading(false);
    };

    const handleSubmitForm = async (formik: FormikProps<IStockCountProps>) => {
        setIsSaving(true);
    ...
    const handleDelete = async (id: number) => {
        var f = await Delete(STOCKCOUNT_URLS.Delete + '?id=' + id);
        if (f) setStockCounts((b) => b.filter((x) => x.id !== id));
    };

    const handleSearchForm = async () => {
        const trans = await getListStocks({
            refNo: ''
        } as IStockCountFilterProps);
        setStockCounts(trans);
    };
    ...
    const handleCloseModal = () => {
        setFormikValues({} as IStockCountProps);
        setState(false);
        setIsSaving(false);
    }
                ...
                <>
                    <div className='row'>
                        <div className='col-8 mb-4'>
                            <div className='display-4 fw-bold py-3'>{t('menu:StockCounts') as ReactNode}</div>
                        </div>
                    ...
                    <Formik
                        ...
                        onSubmit={(values) => { }}
                        validate={(values) => {
                            const errors: Partial<IStockCountProps> = {};
                            //if (!values.amount) {
                            //    errors.amount = 5;
                            //}
                            return errors;
                            ...
                            return (
                                <Modal
                                    ...
                                    <ModalHeader setIsOpen={handleCloseModal}>
                                        <ModalTitle id='exampleModalLabel'>Stock Counts</ModalTitle>
                                    </ModalHeader>
                                    <ModalBody>
                                                            ...
                                                            <table className="table jz-table-bordered">
                                                                ...
                                                                <tbody id="detailBody">
                                                                    {values.detail?.map((d, i) => (
                                                                        <tr key={i}>
                                                                            ...
                                                                            <td>{d?.productName}</td>
                                                                            <td>{d.expected}</td>
                                                                            <td> <input id={`detail.${i}.counted`} type="number" className="form-control" value={d?.counted} onChange={formiki.handleChange} /> </td>
                                                                ...
                                                                <tfoot>
                                                                    <tr className="fw-bolder">
                                                                        ...
                                                                        <th></th>
                                                                        <th>Total</th>
                                                                        <th>{formiki.values.detail?.reduce((total, i) => total + i.counted, 0)}</th>
                                                                        <th></th>
                                                                        <td> </td>
                                                                    </tr>
                                                                </tfoot>
